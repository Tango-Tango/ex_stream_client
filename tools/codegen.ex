# Inspired by original code from https://github.com/dvcrn/ex_openai
# Copyright (c) 2023 Original Author Name
# Licensed under the MIT License. See THIRD_PARTY_NOTICES for details.

defmodule ExStreamClient.Tools.Codegen do
  require Logger
  @moduledoc false
  ...
  """
  def module_overwrites, do: [ExStreamClient.Model]

  # Extracts the group name from a given URL.
  defp extract_group_from_url(url) do
    parts = String.split(url, "/") |> Enum.filter(&(&1 != "")) |> Enum.drop(2)

    case parts do
      ["chat" | rest] when rest != [] ->
        # For chat paths, include the next segment in the group name
        next_segment =
          rest |> Enum.at(0) |> String.split("-") |> Enum.map(&String.capitalize/1) |> Enum.join()

        "Chat.#{next_segment}" |> Macro.underscore()

      _ ->
        # Default case - just use the first segment after api/v2
        parts
        |> Enum.at(0)
        |> String.split("-")
  ...
  ```elixir
  assert ExStreamClient.Tools.Codegen.parse_type(%{
   "type" => "object",
   "properties" => %{
     "foo" => %{
       "type" => "array",
  ...
  def parse_property(%{"anyOf" => anyOf} = args) do
    %{
      ...
      type: {:anyOf, Enum.map(anyOf, fn x -> ExStreamClient.Tools.Codegen.parse_type(x) end)}
    }
  end
  ...
  def parse_property(%{"oneOf" => oneOf} = args) do
    %{
      ...
      description: Map.get(args, "description"),
      required: Map.get(args, "required", false),
      type: {:oneOf, Enum.map(oneOf, fn x -> ExStreamClient.Tools.Codegen.parse_type(x) end)}
    }
  end
  ...
  def parse_property(%{"allOf" => allOf} = args) do
    %{
      ...
      description: Map.get(args, "description"),
      required: Map.get(args, "required", false),
      type: {:allOf, Enum.map(allOf, fn x -> ExStreamClient.Tools.Codegen.parse_type(x) end)}
    }
  end
  ...
  def parse_property(%{
    ...
    %{
      ...
      type: "anyOf",
      required: required,
      oneOf: Enum.map(anyOf, fn x -> ExStreamClient.Tools.Codegen.parse_type(x) end)
    }
  end
  ...
  def parse_component_schema(args), do: parse_component_schema(args |> Map.put("properties", %{}))
  ...
  defp extract_response_type(%{"201" => %{"content" => content}}),
    do: extract_response_type_impl(content)
  ...
  defp extract_response_type_impl(content) do
    return_types =
      content
      # [["application/json", %{}]]
      |> Map.to_list()
  ...
  @spec get_documentation(String.t()) :: %{components: any(), functions: list()}
  def get_documentation(path) do
    {:ok, yml} =
      File.read!(path)
      ...
      functions:
        ...
        |> Enum.map(fn {path, field_data} ->
          ...
          Enum.map(field_data, fn {verb, args} ->
            ...
            parse_path(path, %{verb => Map.put_new(args, "summary", "")}, component_mapping)
          end)
        end)
        |> List.flatten()
  ...
  def string_to_component(comp), do: Module.concat(ExStreamClient.Model, comp)
          ...
          {:component, comp} ->
            ...
            case matching_keys do
              ...
              _ ->
                ...
                {:ok,
                 struct(
                   ExStreamClient.Tools.Codegen.string_to_component(comp),
                   ExStreamClient.Tools.Codegen.keys_to_atoms(res)
                 )}
            end
